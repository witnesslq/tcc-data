...
infosoftglobal.FusionCharts.prototype = {
	...
	getSWFHTML: function() {
		...
		if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) { 
			// netscape plugin architecture	//解决flash覆盖ext组件的问题 加入 wmode='transparent'__by xugengxian_20090927		
			...
			//解决flash覆盖ext组件的问题 加入 wmode='transparent' //by xugengxian_20090927
	...
	render: function(elementId){
		...
		if((this.detectFlashVersion==1) && (this.installedVer.major < 8)){
			...
			//	var installationConfirm = window.confirm("您需要 Adobe Flash Player 8 (或更高版本) 才能查看flash. 服务器已经提供了合适的版本. 请点击‘确定’下载并安装flash浏览器插件.");
				var installationConfirm = window.confirm("您需要 Adobe Flash Player 8 (或更高版本) 才能查看图形报表。这里提供了一个来自Adobe.com轻量级的安装，请点击‘确定’下载并安装该flash浏览器插件。");
				if (installationConfirm){
				window.location = "http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash";
				//	window.location = "/ccic/components/fusionChart/swflash.cab";
				}else{
					return false;
				}
			}else{
