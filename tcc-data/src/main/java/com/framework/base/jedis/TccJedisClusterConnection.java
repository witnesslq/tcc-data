 ...
 * @since 1.7
 */
public class TccJedisClusterConnection implements RedisClusterConnection {
	...
	private volatile TccJedisSubscription subscription;

	/**
	 * Create new {@link TccJedisClusterConnection} utilizing native connections via {@link JedisCluster}.
	 * 
	 * @param cluster must not be {@literal null}.
	 */
	public TccJedisClusterConnection(JedisCluster cluster) {
	...
	/**
	 * Create new {@link TccJedisClusterConnection} utilizing native connections via {@link JedisCluster} running commands
	 * across the cluster via given {@link ClusterCommandExecutor}.
	 *
	 * @param cluster must not be {@literal null}.
	 * @param executor must not be {@literal null}.
	 */
	public TccJedisClusterConnection(JedisCluster cluster, ClusterCommandExecutor executor) {
	...
	public void mSet(Map<byte[], byte[]> tuples) {
		...
		for (Entry<byte[], byte[]> entry : tuples.entrySet()) {
	...
	public Cursor<Entry<byte[], byte[]>> hScan(final byte[] key, ScanOptions options) {
		...
		return new ScanCursor<Entry<byte[], byte[]>>(options) {
			...
			protected ScanIteration<Entry<byte[], byte[]>> doScan(long cursorId, ScanOptions options) {
				...
				redis.clients.jedis.ScanResult<Entry<byte[], byte[]>> result = cluster.hscan(key, JedisConverters.toBytes(cursorId), params);
				return new ScanIteration<Entry<byte[], byte[]>>(Long.valueOf(result.getStringCursor()), result.getResult());
			}
		}.open();
	}
	...
	public void subscribe(MessageListener listener, byte[]... channels) {
		...
		try {
			BinaryJedisPubSub jedisPubSub = new TccJedisMessageListener(listener);
			subscription = new TccJedisSubscription(listener, jedisPubSub, channels, null);
	...
	public void pSubscribe(MessageListener listener, byte[]... patterns) {
		...
		try {
			BinaryJedisPubSub jedisPubSub = new TccJedisMessageListener(listener);
			subscription = new TccJedisSubscription(listener, jedisPubSub, null, patterns);
	...
	public void openPipeline() {
		throw new UnsupportedOperationException("Pipeline is currently not supported for TccJedisClusterConnection.");
	}
	...
	public RedisSentinelConnection getSentinelConnection() {
		throw new UnsupportedOperationException("Sentinel is currently not supported for TccJedisClusterConnection.");
	}
