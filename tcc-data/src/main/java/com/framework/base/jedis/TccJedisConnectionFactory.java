 ...
 * @author Mark Paluch
 */
public class TccJedisConnectionFactory implements InitializingBean, DisposableBean, RedisConnectionFactory {

	private final static Log log = LogFactory.getLog(TccJedisConnectionFactory.class);
	...
	/**
	 * Constructs a new <code>TccJedisConnectionFactory</code> instance with default settings (default connection pooling, no
	 * shard information).
	 */
	public TccJedisConnectionFactory() {}

	/**
	 * Constructs a new <code>TccJedisConnectionFactory</code> instance. Will override the other connection parameters passed
	 * to the factory.
	 * 
	 * @param shardInfo shard information
	 */
	public TccJedisConnectionFactory(JedisShardInfo shardInfo) {
		this.shardInfo = shardInfo;
	}

	/**
	 * Constructs a new <code>TccJedisConnectionFactory</code> instance using the given pool configuration.
	 * 
	 * @param poolConfig pool configuration
	 */
	public TccJedisConnectionFactory(JedisPoolConfig poolConfig) {
		this((RedisSentinelConfiguration) null, poolConfig);
	}

	/**
	 * Constructs a new {@link TccJedisConnectionFactory} instance using the given {@link JedisPoolConfig} applied to
	 * {@link JedisSentinelPool}.
	 * 
	 * @param sentinelConfig
	 * @since 1.4
	 */
	public TccJedisConnectionFactory(RedisSentinelConfiguration sentinelConfig) {
	 ...
	 * @since 1.4
	 */
	public TccJedisConnectionFactory(RedisSentinelConfiguration sentinelConfig, JedisPoolConfig poolConfig) {
		this.sentinelConfig = sentinelConfig;
		this.poolConfig = poolConfig != null ? poolConfig : new JedisPoolConfig();
	}

	/**
	 * Constructs a new {@link TccJedisConnectionFactory} instance using the given {@link RedisClusterConfiguration} applied
	 * to create a {@link JedisCluster}.
	 * 
	 * @param clusterConfig
	 * @since 1.7
	 */
	public TccJedisConnectionFactory(RedisClusterConfiguration clusterConfig) {
	 ...
	 * @since 1.7
	 */
	public TccJedisConnectionFactory(RedisClusterConfiguration clusterConfig, JedisPoolConfig poolConfig) {
		this.clusterConfig = clusterConfig;
	 ...
	 */
	protected TccJedisConnection postProcessConnection(TccJedisConnection connection) {
		return connection;
	}
	...
	/**
	 * Creates {@link TccJedisPool}.
	 * 
	 * @return
	 * @since 1.4
	 */
	protected Pool<Jedis> createRedisPool() {
		return new TccJedisPool(getPoolConfig(), getShardInfo().getHost(), getShardInfo().getPort(),
	...
	private JedisCluster createCluster() {
		...
		this.clusterCommandExecutor = new ClusterCommandExecutor(new TccJedisClusterConnection.JedisClusterTopologyProvider(
				cluster), new TccJedisClusterConnection.JedisClusterNodeResourceProvider(cluster), EXCEPTION_TRANSLATION);
		return cluster;
	}
	...
	public RedisConnection getConnection() {
		...
		TccJedisConnection connection = (usePool ? new TccJedisConnection(jedis, pool, dbIndex) : new TccJedisConnection(jedis,
				null, dbIndex));
	...
	/*
	 * (non-Javadoc)
	 * @see org.springframework.mapper.support.PersistenceExceptionTranslator#translateExceptionIfPossible(java.lang.RuntimeException)
	 */
	...
	/**
	 * Specifies if pipelined results should be converted to the expected data type. If false, results of
	 * {@link org.springframework.data.redis.connection.jedis.JedisConnection#closePipeline()} and {@link TccJedisConnection#exec()} will be of the type returned by the
	 * Jedis driver
	 * 
