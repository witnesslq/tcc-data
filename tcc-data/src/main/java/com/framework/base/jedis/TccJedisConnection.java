 ...
 * @author Mark Paluch
 */
public class TccJedisConnection extends AbstractRedisConnection {
	...
	/**
	 * Constructs a new <code>TccJedisConnection</code> instance.
	 *
	 * @param jedis Jedis entity
	 */
	public TccJedisConnection(Jedis jedis) {
		this(jedis, null, 0);
	}

	/**
	 * Constructs a new <code>TccJedisConnection</code> instance backed by a jedis pool.
	 *
	 * @param jedis
	 * @param pool can be null, if no pool is used
	 */
	public TccJedisConnection(Jedis jedis, Pool<Jedis> pool, int dbIndex) {
		this.jedis = jedis;
	...
	public Cursor<byte[]> scan(long cursorId, ScanOptions options) {
		...
		return new ScanCursor<byte[]>(cursorId, options) {
			...
			protected ScanIteration<byte[]> doScan(long cursorId, ScanOptions options) {
				...
				ScanResult<String> result = jedis.scan(Long.toString(cursorId), params);
	...
	public Cursor<byte[]> sScan(byte[] key, long cursorId, ScanOptions options) {
		...
		return new KeyBoundCursor<byte[]>(key, cursorId, options) {
			...
			protected ScanIteration<byte[]> doScan(byte[] key, long cursorId, ScanOptions options) {
				...
				ScanResult<byte[]> result = jedis.sscan(key, JedisConverters.toBytes(cursorId), params);
